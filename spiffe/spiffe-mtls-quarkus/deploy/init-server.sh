...
kubectl create namespace server || true

# ServiceAccount for hello-server workload attestation by SPIRE Agent
kubectl apply -f - <<EOF
apiVersion: v1
kind: ServiceAccount
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      # UID 185 (ubi9/openjdk-21 default) and cannot read them without this step.
