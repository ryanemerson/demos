...
- **Frontend** — Quarkus web app with Keycloak SSO login (authorization code flow). Accepts browser TLS connections (no client cert required). Communicates with Keycloak and downstream services over mTLS. At startup, generates a fake TLS certificate (matching the SPIFFE CA's distinguished name but signed by a different key) to demonstrate mTLS rejection scenarios.
...
| Scenario | Bearer Token | TLS Certificate | Rejection Layer | Expected Result |
|----------|-------------|-----------------|-----------------|-----------------|
| **Sender Constrained Token** | Valid (Keycloak-issued, `cnf`-bound) | Valid SPIFFE X.509-SVID | — | Success (if user has required roles) |
| **Mocked Bearer Token** | Fabricated (no valid signature) | Valid SPIFFE X.509-SVID | Application (HTTP 401) | Rejected by Microservice 1 token validation |
| **Invalid mTLS** | Valid (Keycloak-issued, `cnf`-bound) | Fake cert (matching CA DN, wrong signing key) | TLS (`certificate_unknown` alert) | Rejected before request reaches the application |

The **Invalid mTLS** scenario uses a certificate generated at container startup. The entrypoint script reads the real SPIFFE CA's distinguished name from the trust bundle and creates a fake CA with the same DN, then issues a client certificate from it. The certificate's issuer matches what the server expects, so the client presents it during the TLS handshake, but the server cannot verify the signature against the real SPIFFE CA — resulting in a `certificate_unknown` TLS alert.
...
5. Try each scenario:
   ...
   - **Invalid mTLS** — demonstrates TLS-layer rejection when the client certificate is not trusted by the server's SPIFFE trust bundle
...
├── frontend/
│   ├── Dockerfile
│   ├── entrypoint.sh               # Generates fake mTLS cert at startup, then starts Quarkus
│   ├── spiffe-helper.conf
