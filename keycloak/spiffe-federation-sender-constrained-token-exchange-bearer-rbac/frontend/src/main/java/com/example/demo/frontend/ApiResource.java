...
public class ApiResource {
    ...
    @Inject
    @RestClient
    Microservice1InvalidMtlsClient microservice1InvalidMtlsClient;
    ...
    @GET
    @Path("/call-service-invalid-mtls")
    @Produces(MediaType.TEXT_PLAIN)
    public String callServiceInvalidMtls() {
        String currentToken = accessToken.getRawToken();
        String authHeader = "Bearer " + currentToken;

        var sb = new StringBuilder();
        sb.append("=== Frontend (Invalid mTLS) ===\n\n");
        sb.append("--- Outgoing JWT (Bearer Token) ---\n");
        sb.append(prettyPrintJwt(currentToken)).append("\n\n");
        sb.append("Using a self-signed certificate NOT part of the SPIFFE CA.\n");
        sb.append("The mTLS handshake with Microservice 1 should be rejected.\n\n");

        try {
            Response response = microservice1InvalidMtlsClient.call(authHeader);
            ...
            sb.append("mTLS handshake rejected: ").append(e.getMessage()).append("\n");
        }
